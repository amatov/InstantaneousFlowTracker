// Goldberg AV. 1997. An Efficient Implementation of A Scaling Minimum-Cost
// Flow Algorithm. J.Algorithms 22:1-29
// AM 31.01.2003
...
// IT LOOKS LIKE THE PROGRAM STARTS HERE
              ...
              long *cap_p)   /* array of capacities */
{
  ...
  empty_push_bound = (long)(n * EMPTY_PUSH_COEF);
    ...
    while ( NONEMPTY_STACKQ ) {
      ...
      FOR_ALL_ARCS_a_FROM_i
      {
        ...
        rc = REDUCED_COST ( i, j, a );
          ...
          {
            dr = ( - rc - 0.5 ) / epsilon;
            if (( j_rank = (long)(dr + i_rank) ) < dlinf ) {
              if ( j_rank > j -> rank )
                j -> rank = j_rank;
            }
          }
