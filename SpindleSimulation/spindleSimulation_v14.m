...
intStdTrack = 5; % std of speckle intensity over a single track
...
for i = 2 : totalFrameNum
    ...
    for j = 1 : previousFrameTrackNum  % propagation is completely track-based.
        ...
        tempInt = track(j).points(track(j).len, 3) + randn(1,1) * intStdTrack; 
        if (tempInt < 0)
            tempInt = 0;
        end
...
for i = 1 : totalFrameNum
    fprintf('Total number of speckles is %d in image %d\n', frameStack(i).length, i);
        for j = 1 : frameStack(i).length
             ...
             temp_center = round(frameStack(i).coordinate(j, 1:3));
             fprintf(fid,' (');
             fprintf(fid, '%10.6f %10.6f %10.6f', temp_center(2), temp_center(1), temp_center(3));
             fprintf(fid,')');
             if (mod(j,5) == 0)
                 fprintf(fid,' \n');
             end
