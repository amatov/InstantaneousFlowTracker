...
%--------------------------------------------------------------------------
intMean = 180;     % mean value of speckle intensity over the entire image
intStdImg = 10;    % std value of speckle intensity over the entire image
    ...
    for m = 1 : previousFrameTrackNum
       ...
       for n = 1  : previousFrameTrackNum
           ...
           if (EuclideanDistance(track(m).points(L1, 1:2), track(n).points(L2, 1:2)) < minPtDist)
               if (track(m).status ~= 0) % it is possible that more than 2 tracks merge.
...
for i = 1 : totalFrameNum
    fprintf('Total number of speckles is %d in image %d\n', frameStack(i).length, i);
        for j = 1 : frameStack(i).length
             ...
             tempData = round(frameStack(i).coordinate(j, 1:3));
             fprintf(fid,' (');
             fprintf(fid, '%10.6f %10.6f %10.6f', tempData(1), tempData(2), tempData(3));
             fprintf(fid,')');
...
y_grid_num = round((y_range - 1)/ y_grid_size);
x_grid_num = round((x_range - 1)/ x_grid_size);
...
for i = 1 : y_grid_num
    for j = 1 : x_grid_num
        ...
        fprintf(fid,' (');
        fprintf(fid, '%10.6f %10.6f', tempFlowVec(1), tempFlowVec(2));
        fprintf(fid,')');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Step 7: Computing basic statistics of the synthetic data for verification
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
totalPtNum1 = 0;
...
for i = 1 : totalFrameNum
    ...
    for j = 1 : frameStack(i).length
        ...
        if ((temp_center(1) > speckle_size_half) &...
            ...
            temp_center(2) - speckle_size_half : temp_center(2) + speckle_size_half, 2) = round(frameStack(i).coordinate(j, 3));  % use green to represent general points
        end
    end
