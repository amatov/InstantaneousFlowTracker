...
%--------------------------------------------------------------------------
start_path = 'D:\manTrackWT2s\speckles\';
ptNum = getSpeckleNum(start_path);
ptNum = round(ptNum * 0.7);

%--------------------------------------------------------------------------
% Step 2.2: Distance control parameters
%--------------------------------------------------------------------------
minPtDist = 7;  % Points go within this distance to each other will be merged.
...
intStdImg = 35;    % std value of speckle intensity over the entire image
intStdTrack = 10;   % std of speckle intensity over a single track. 
...
while (currentPtNum < ptNum(1))       % Need to fill the whole array
    ...
    if (failedGenerationNum > maxAllowedFailureNum)
        fprintf('WARNING: Maximum number of failure reached. Required number of points is NOT reached.\n');
        break;
    end
...
magVec = [1.75 0.25];  % First element is mean, second is std
thetaVec = [0 20];     % in degrees. First element is mean. Second is std
...
%            
        ...
        if (failedGenerationNum > maxAllowedFailureNum)
            fprintf('WARNING: maximum failure number reached, required point number can not be reached. \n');
        end
...
fileName4 = 'intensityProfile.txt';
...
fileName5 = 'groundtruth_trackData001.txt';
...
for i = 1 : totalTrackNum
    fprintf(fid, '(');
    fprintf(fid, '%f %f %f', track(i).startID, track(i).len, 0);
    fprintf(fid, ')\n');
...
temp_image_grey = zeros(imgHeight, imgWidth);


for i = 1 : totalFrameNum
    temp_image (1:imgHeight, 1:imgWidth, 1:3) = 0;
    temp_image_grey(1 : imgHeight, 1 : imgWidth) = 0;
    speckle_size_half = 1;
    ...
    for j = 1 : frameStack(i).length
        ...
        if ((temp_center(1) > speckle_size_half) &...
            ...
            temp_image_grey(temp_center(1) - speckle_size_half : temp_center(1) + speckle_size_half,... 
            temp_center(2) - speckle_size_half : temp_center(2) + speckle_size_half) = round(frameStack(i).coordinate(j, 3));
        
        end
    end
    image_stack(:,:,:,i) = uint8(temp_image);
    TIFfileName = [path2, 'simspindle', num2str(i), '.tif'];
    imwrite(uint8(temp_image_grey), TIFfileName, 'tif');
end
