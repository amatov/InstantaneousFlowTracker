...
if isempty(nbDir)
    [nbDir,bestpp,bestmu,bestcov,dl,countf] = mixtures4_circular(ang);
end
...
%    [cluster_index, bestmu] = kmeans(ang, nbDir,'emptyaction','singleton');
    [bestk,bestpp,bestmu,bestcov,dl,countf] = mixtures4_circular_known_K(ang,nbDir);
    bestmu * 180 / pi
    bestpp
    [bestcov(:)]'
...
%         aux=vecFldInterpAnisoA([fYf,fXf,fYtf,fXtf],[fYf,fXf], flowVecList, 40,10);
        [aux,vecDir]=vecFldInterpAnisoB([fYf,fXf,fYtf,fXtf],[fYf,fXf],12,3,3);%40 10 / 16-4
