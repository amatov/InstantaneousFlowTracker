function [trackedLinks, flowHistory]=andreaTest_v3_readsFromFeat_cleaned(inpDir,tifImagesDir,dist,firstTriFrame,lastTriFrame,s,nbDir,rate,pixelS,name,RAK_ANGLE, arrowScale)
...
for j = firstTriFrame:lastTriFrame
    ...
    %     nbDir = 2;
    %     RAK_ANGLE = [];
    [trackedLinks,flowN,cluster_index,bestmu,flowMap,dir] = flowTracker(frames1.pos,frames2.pos,frames3.pos,dist,1,1,1,nbDir,3,1,0,RAK_ANGLE); %3-4 dir % TORSTENS DATA
    ...
    for i = 1:nbDir
        dir(i).list = find(dir(i).map(:,1)>0);
        dir(i).map = dir(i).map(dir(i).list,:);
    end
...
%     
    hold on;
    cMap = {'yellow','red','green','white','magenta'};
    ...
    print(h,'-djpeg100',[tifImagesDir,filesep,'resultsROI2',filesep,'vectorMap-',name,indxStr,'.jpg']);
