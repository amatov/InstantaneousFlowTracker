function [links,flow,cluster_index,gmm,flowMap,dir] = flowTracker_v4(frames,dist,w1,w2,w3,nbDir,iterations,firstF,nbWindows,RAK_ANGLE, filterSigmaU)
...
for i = firstF:firstF+nbWindows
    Graph(i).Ipos = frames(i).pos;
    Graph(i).Jpos = frames(i+1).pos;
    Graph(i).Kpos = frames(i+2).pos;
end
...
for j = 1:iterations
    costAdd = [];cAdd = [];cluster_index = [];
    
    if nbDir == 1 %2
        [cAdd,w3,cluster_index,gmm,flowMap,dir] = clusteringWrap(links,nbDir,ve,w3,singleTriplets,RAK_ANGLE); % 2 DIR
    else
        [cAdd,w3,cluster_index,gmm,flowMap,dir,nbDir] = clusteringKmeans_v4(links,nbDir,ve,w3,singleTriplets, filterSigmaU); % 3 DIR
    end
    ...
    flow = Graph(end).flow;
    % NEW 2009 NOV
    [cAdd,w3,cluster_index,gmm,flowMap,dir] = clusteringKmeans_v4(links,nbDir,ve,w3,singleTriplets, filterSigmaU);
end