function [interField,prevFlowVecList]=vecFldInterpAnisoB(M, tgtPoints, sigmaU, sigmaV, maxIteration, polygon)
...
% DEPENDENCES                 This function calls vecFldInterpAnisoB
%
...
%fprintf('Distance matrix computation done.\n');
...
threshold = 1e-2; 
%fprintf('Initial vector field computation done.\n');

i = 0;
while (1)
    newFlowVecList = vecFldInterpAnisoA(M, tgtPoints, prevFlowVecList, sigmaU, sigmaV);
    if size(M,1) == 0
        newFlowVecList = zeros(size(prevFlowVecList));
    end
    error = sqrt(sum(newFlowVecList - prevFlowVecList).^2) / tgtLength;
    if (error < threshold)
        interField = newFlowVecList;
 %       fprintf('Computation converged in flow field calculation. Now return to the calling point.\r');
        return;
    else
        i = i + 1;
        if (i >= maxIteration)
